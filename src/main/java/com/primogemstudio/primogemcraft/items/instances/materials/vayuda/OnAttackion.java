        ...
        if (itemstack.getItem() instanceof VayudaTurquoiseGemstoneIronSwordItem) {
            if (Math.random() < 0.25) {
                ...
                if (!level.isClientSide()) {
                    level.playSound(null, new BlockPos((int) x, (int) y, (int) z), SoundEvents.TRIDENT_RIPTIDE_2, SoundSource.PLAYERS, (float) 0.5, (float) Mth.nextDouble(RandomSource.create(), 1.5, 3));
                }
                else {
                    level.playLocalSound(x, y, z, SoundEvents.TRIDENT_RIPTIDE_2, SoundSource.PLAYERS, (float) 0.5, (float) Mth.nextDouble(RandomSource.create(), 1.5, 3), false);
                }
            }
        }
