...
public class YibangrenzhinangInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == PrimogemcraftModItems.YINGJICUNCHUNANG.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(143) {
			@Override
